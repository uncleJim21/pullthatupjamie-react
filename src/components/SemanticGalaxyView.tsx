...
// Shift hue of a hex color by `degrees`. For achromatic colors (like white)
// we inject a small amount of saturation so the shift is visible.
const shiftHue = (hexColor: string, degrees: number): string => {
  const hex = hexColor.replace('#', '');
...
const Star: React.FC<StarProps> = ({ result, isSelected, isNearSelected, hasSelection, isSpotlightActive, isKeywordMatch, hasKeywordFilter, onClick, onRightClick, onHover, overrideColor }) => {
  ...
  // Shift hue when selected so the star is visibly distinct from its unselected neighbors.
  const rawColor = overrideColor || getHierarchyColor(result.hierarchyLevel);
  const baseColor = isSelected ? shiftHue(rawColor, SELECTED_HUE_SHIFT) : rawColor;
                    ...
                    {researchSessionItems.length > 0 ? (
                      ...
                      <div className="text-[0.65rem] text-gray-500 text-center py-2">
                        ...
                        <div className="text-[0.6rem] text-gray-600 mt-1">
                          Right-click or use stack button
                        </div>
                      </div>
                    )}
                  </>
